                ...
                <tbody>
                    @foreach ($quizzes as $quiz)
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2">{{ $quiz->user->name }}</td>
                            <td class="px-4 py-2 capitalize">{{ $quiz->status }}</td>
                            <td class="px-4 py-2">{{ $quiz->score ?? '—' }}</td>
                            <td class="px-4 py-2">{{ \Carbon\Carbon::parse($quiz->started_at)->diffForHumans() ?? '—' }}
                            </td>
                            <td class="px-4 py-2">{{ \Carbon\Carbon::parse($quiz->started_at)->diffForHumans() ?? '—' }}
                            </td>
                            <td class="px-4 py-2 space-x-2">
                                ...
                                <button type="button" class="text-red-600 hover:underline"
                                    @click="openModal('{{ route('admin.quizzes.destroy', $quiz) }}')">
                                    Delete
                                </button>
                            </td>
                        </tr>
